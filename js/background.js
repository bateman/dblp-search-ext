...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    try {
        if (message.type === 'REQUEST_SEARCH_PUBLICATIONS') {
            ...
            controller.handleSearch(message.query).then(() => {
                console.log(`Background.js completed a request to search for '${message.query}' sent by '${message.script}'.`);
                sendResponse({ script: 'background.js', response: `Search for '${message.query}' completed.` });
            }).catch(error => {
                console.error(`There was a problem with the message '${message.type}' sent by '${message.script}': ${error}`);
                sendResponse({ script: 'background.js', response: `There was a problem with the message '${message.type}' sent by '${message.script}': ${error}` });
            });
...
browser.contextMenus.onClicked.addListener(function (info) {
    if (info.menuItemId === "selection") {
        ...
        browser.tabs.create({ url: 'https://dblp.org/search?q=' + encodeURIComponent(query) });
    }
});